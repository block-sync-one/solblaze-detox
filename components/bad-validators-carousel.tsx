...
function filterValidatorsByPolicy(validators: Validator[], maxPerPolicy: number = 30): Validator[] {
...
export default function BadValidatorsCarousel({ badValidators }: { badValidators: Validator[] }) {
  ...
  useEffect(() => {
    ...
    const filteredValidators = filterValidatorsByPolicy(badValidators.filter(validator => validator.policy && validator.policy.trim()), 30)
    console.log('filteredValidators', badValidators.filter(validator => validator.policy && validator.policy.trim()), 30)
      ...
      <div className="flex items-center space-x-3 text-text/70">
        ...
        <div className="flex items-center space-x-2 font-mono">
          <span className="font-semibold text-red-400 transition-all duration-300">
            {currentValidator.name || shortenAddress(currentValidator.voteAccount)}
          </span>
          <span className="text-text/60">
            {currentValidator.policy}
          </span>
        </div>
      </div>
