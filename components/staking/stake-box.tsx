...
export default function StakeBox({ validators }: { validators: Validator[] }) {
  ...
  const [selectedAccount, setSelectedAccount] = useState<StakeInfo | null>(
    null,
  );
  ...
  const memoizedValidators = useMemo(
    () => validators,
    [validators.length, validators.map((v) => v.voteAccount).join(",")],
  );
  ...
  const stakePoolExchangeRate = useCallback(async () => {
    const allPools = await fetch(
      "https://api.solanahub.app/api/stake/get-stake-pools",
    );
  ...
  const handleDelegate = useCallback(async () => {
    ...
    try {
      // @ts-ignore
      const [instructions, signers] = await delegateStake(
        selectedAccount,
        publicKey,
        connection,
      );
      ...
      <div className="bg-secondary/30 backdrop-blur-sm rounded-2xl shadow-xl border border-primary/20 p-8">
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <p className="text-text/70 font-light">
              Receive bSOL tokens for your delegation
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            <div>
              <div className="block text-sm font-semibold text-text/90 mb-3">
        ...
        <div className="mt-6 flex items-center justify-center gap-2 text-xs text-text/50">
          ...
          <span className="font-light max-w-sm text-center">
            Your stake will be partially delegated to solanahub validator as a
            gesture for building this public good app
          </span>
        </div>
      </div>
    </NeonGradientCard>
