...
export function aggregateStakeAccount(account: any, validators: Validator[], currentEpoch: number): Partial<StakeInfo> {
  ...
  const isActive = Number(info.stake?.delegation?.activationEpoch) < currentEpoch;
  const findValidator = validators.find(v => v.voteAccount == validator);
  console.log('activationEpoch', validator, info.stake?.delegation?.activationEpoch, currentEpoch, isActive)
  return {
