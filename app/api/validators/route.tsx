...
async function fetchPolicyValidators(
  policy: Policies,
  stakeWizValidators: StakeWizValidator[],
  ...
  try {
    ...
    return policyData.validators.map((identity: string) => {
      ...
      return {
        ...
        policy: policy.name,
        warning:
          policy.name === "Sandwicher List"
            ? "MEV sandwiching detected"
            : "Slow block producers",
      };
    });
  } catch (error) {
...
function getHighCommissionValidators(
  stakeWizValidators: StakeWizValidator[],
): ValidatorResult[] {
  ...
  return stakeWizValidators
    .filter(
      (validator: StakeWizValidator) => validator.commission > maxCommission,
    )
    .map(
      (validator: StakeWizValidator): ValidatorResult => ({
...
function getGoodValidators(
  stakeWizValidators: StakeWizValidator[],
  badValidators: ValidatorResult[],
): ValidatorResult[] {
  const badValidatorVoteAccounts = new Set(
    badValidators.map((validator) => validator.voteAccount),
  );
...
function combineAndDeduplicateValidators(
  validatorArrays: ValidatorResult[][],
): ValidatorResult[] {
  const allValidators = validatorArrays.flat();

  return allValidators.reduce(
    (acc: ValidatorResult[], current: ValidatorResult) => {
      const existing = acc.find(
        (validator) => validator.voteAccount === current.voteAccount,
      );
...
export async function GET(request: Request) {
  try {
    ...
    const stakeWizValidators: StakeWizValidator[] = await (
      await fetch("https://api.stakewiz.com/validators")
    ).json();
    ...
    const highCommissionValidators =
      getHighCommissionValidators(stakeWizValidators);
